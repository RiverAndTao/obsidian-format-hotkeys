...
另外可通过命令面板使用：加粗、格式转加粗、取消变红、去除空行、插入顿号、块级公式转行内、中英文标点转换等。
...
- **加粗**：选中文字切换 `**加粗**`
- **格式转加粗**：把选区内红字 / `<font>` / `<span style="color">` / 下划线全部变成 `**加粗**`
- **取消变红**：去掉选区内变红标记（原先带粗体的红字会留下 `**加粗**`）
...
- 正文字体对齐 Typora GitHub（Open Sans + 微软雅黑 UI），红字用 GitHub 危险色并随深色主题切换
- `arm` / `asm` 代码块按 Typora CodeMirror 配色高亮（浅色 GitHub、深色 Night）
